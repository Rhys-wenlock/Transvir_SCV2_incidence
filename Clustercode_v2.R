...
single_index_HH_1 <- single_index_HH_1 %>% 
  ...
  select(Household_ID, Index_ID, Ct_val, symp)
...
df <- data.frame(
  index_id = c(101, 102, 103, 104, 105),
  Date = as.Date(c('2023-09-10', '2023-09-11', '2023-09-12', '2023-09-12', '2023-09-13')),
  Household_ID = c('H1', 'H1', 'H2', 'H2', 'H2'),
  cluster_no = c(1, 2, 1, 1, 2)
)

# Creating a distinct table for each combination and assigning a unique cluster_ID
distinct_clusters <- df %>%
  select(Household_ID, cluster_no) %>%
  distinct() %>%
  arrange(Household_ID, cluster_no) %>%
  mutate(cluster_ID = row_number())

# Joining back to the original dataframe to get the cluster_ID for each row
df <- left_join(df, distinct_clusters, by = c("Household_ID", "cluster_no"))
