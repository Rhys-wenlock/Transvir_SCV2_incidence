...
summary(total_index_data$cluster_no)
table(total_index_data$S_Ind)
table(total_index_data$count, total_index_data$S_Ind)
...
table(total_single_index_data$symp)
summary(total_single_index_data$Ct_val)
table(total_single_index_data$age_cat)
...
summary_cluster_data <- total_single_cluster_data %>% 
  group_by(cluster_ID) %>%
  summarise(
    num_participants = n(),
...
sum(summary_cluster_data$num_participants)
summary(summary_cluster_data$num_participants)
sum(summary_cluster_data$num_positive)
table(summary_cluster_data$num_positive)
