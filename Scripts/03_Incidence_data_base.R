...
inf_vaccine_serol_FC_countinf <- read_csv(here::here("Data", "inf_vaccine_serol_FC_medianSN_medianNvacc.csv"))
...
#Need to have run PCR_negative_dates.R and have sample_weeks function in environment
...
write.csv(all_inf_data, "all_inf_data_base.csv")
...
gambia_case_linelist <- read.csv(here::here("Data", "gambia_case_linelist.csv"))
colnames(gambia_case_linelist)[2] <- "Type"
gambia_case_linelist$date <- as.Date(gambia_case_linelist$date)
gambia_case_linelist <- gambia_case_linelist %>% 
  select(Type, date)
...
ggsave("supp_fig_1a.png", supp_figure_1a, width = 10, height = 6, dpi=600)
