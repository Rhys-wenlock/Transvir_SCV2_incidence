...
inf_vaccine_serol_FC_countinf <- read_csv(here::here("Data","inf_vaccine_serol_FC_medianSN_medianNvacc_90.csv"))
#Run median SN analysis
...
first_inf <- test %>% filter(date_diff<=90)
...
second_inf <- test %>% filter(date_diff<=90)
...
third_inf <- test %>% filter(date_diff<=90)
...
all_inf_data <- all_inf_data %>% 
  ...
  mutate(start_date = case_when(
    total_inf == 0 ~ V1_date,
    inf_num == 1 ~ V1_date,
    inf_num == 0 & total_inf == 1 ~ First_positive_date + 90,
    inf_num == 2 ~ First_positive_date + 90,
    inf_num == 0 & total_inf == 2 ~ second_positive_date + 90,
    inf_num == 3 ~ second_positive_date + 90,
    inf_num == 0 & total_inf == 3 ~ third_positive_date + 90,
    TRUE ~ NA
  )
  ) %>%
  mutate(
...
write.csv(all_inf_data, "all_inf_data_7.csv")
...
supp_figure_1a <- ggplot(epi_curve) + 
...
ggsave("supp_fig_1h.png", supp_figure_1a, width = 10, height = 6, dpi=600)
